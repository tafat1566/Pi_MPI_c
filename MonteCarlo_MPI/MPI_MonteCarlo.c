...
int main(int argc, char* argv[]){
    ...
    unsigned long long int n_it = 100000000,resultat = 0;            
    ...
    if(myid != 0){                             // Tout le monde envoie sa contribution au nœud 0 
    ...
    if (myid == 0){      
        ...
        done = ((error < epsilon) || ((total_n_it)>1000000));
        request = (done) ? 0 : 1;
        printf("Processeurs = %2d,    Itérations = %ld,    Temps = %f s,\n", nbprocess, total_n_it,total_time);
        printf("pi = %23.20lf\n", pi );
    }
